import { editTrackexp, editTrackSell } from "@/app/api/client/trackingbakery";
...
import { Bakery_sold } from "../../tracksell/(component)/ParentTable";
...
const EditTrackExp = ({ id, setCheckBakery }: EditTrackSellProps) => {
  const handleEditSell = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      await editTrackexp({ quantity: dataToApi.quantity as number }, id);
      setCheckBakery((prev) =>
        prev.map((item) =>
          item.id === id
